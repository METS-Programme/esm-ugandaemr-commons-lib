...
export const MultipleEncounterList: React.FC<MultipleEncounterListProps> = ({
  ...
  const [encountersMap, setEncountersMap] =
    useState<Record<any, Array<any>>>(null);
      ...
      Promise.all(encounterPromises).then((values) => {
        values.forEach(({ data }) => {
          if (data.results?.length > 0) {
            ...
            encountersToTypeMap[sortedEncounters[0].encounterType.uuid] =
              sortedEncounters;
          }
        });
        let baseType = null;
  ...
  useEffect(() => {
    if (baseEncounterType && encountersMap) {
      ...
      const otherTypes = Object.keys(encountersMap).filter(
        (encounterType) => encounterType != baseEncounterType
      );
      const encountersChunck: any[] = baseEncounterArray.map(
        (encounter, index) => {
