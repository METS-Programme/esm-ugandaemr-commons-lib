...
export const EncounterList: React.FC<EncounterListProps> = ({
  ...
  const launcherIsMarkedAsHidden = useMemo(() => {
    return isDead
      ? true
      : launchOptions.hideFormLauncher ?? hideFormLauncher ?? false;
  ...
  const loadRows = useCallback(
    (encounterType) => {
      ...
      openmrsFetch(
        `/ws/rest/v1/encounter?${query}&v=${encounterRepresentation}`
      ).then(({ data }) => {
        if (data.results?.length > 0) {
          ...
          if (filter) {
            sortedEncounters = sortedEncounters.filter((encounter) =>
              filter(encounter)
            );
          }
  ...
  const launchEncounterForm = (
    ...
    const launcherTitle =
      `${capitalize(action)} ` + (form?.name || encounterForm?.name);
  ...
  const formLauncher = useMemo(() => {
    ...
    if (forms && forms.length > 1) {
      encounterForms = forms.map((formV) => {
        ...
        return excludedIntents.length
          ? updateExcludeIntentBehaviour(excludedIntents, tempForm)
          : tempForm;
      });
      ...
      encounterForm.availableIntents &&
